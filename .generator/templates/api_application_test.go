...
func Test_Get_Applications(t *testing.T) {
	resp, err := apiClient.ApplicationAPI.GetApplication(apiClient.cfg.Context, "0oadzm91xatWrYizt1d7").Execute()
	if err != nil {
		t.Error(err)
	}
	d, _ := io.ReadAll(resp.Body)
	t.Error(prettyprintByte(d))
}

func Test_CreateGroups(t *testing.T) {
	resp, err := apiClient.GroupAPI.CreateGroup(apiClient.cfg.Context).Data(`{"profile":{"name":"sbcd"}}`).Execute()
	if err != nil {
		t.Error(err)
	}
	d, _ := io.ReadAll(resp.Body)
